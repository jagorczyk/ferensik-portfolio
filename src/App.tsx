...
function StaticSkinsScene() {
  return <div className="skins-stage" aria-label="Statyczne modele skinów"><Canvas camera={{ position: [0, 6.8, 1.1], up: [0, 0, 1], fov: 32 }} dpr={[1, 1.7]} gl={{ antialias: true }}><color attach="background" args={['#080808']} /><ambientLight intensity={0.42} /><directionalLight position={[3, 4, 5]} intensity={1.15} color="#fff" /><directionalLight position={[-4, 1, 2]} intensity={0.35} color="#666" /><Suspense fallback={null}><group position={[-0.82, 0, 0]} rotation={[0, 0, 0]} scale={1.05}><DffModel modelName="agent" basePath="/skins" materialColor="#b8b8b2" fallback={<FallbackObject />} /></group><group position={[0.82, -0.38, 0]} rotation={[0, 0, 0]} scale={1.05}><DffModel modelName="randomGang" basePath="/skins" materialColor="#b8b8b2" fallback={<FallbackObject />} /></group><Environment preset="studio" /><ContactShadows position={[0, 0, -1.25]} opacity={0.82} scale={4.4} blur={2.2} /></Suspense></Canvas></div>
}
