...
function FallbackObject() {
  return <group><mesh rotation={[0.2, 0, 0]}><icosahedronGeometry args={[1.45, 2]} /><meshStandardMaterial color="#e9e9e9" roughness={0.28} metalness={0.3} wireframe /></mesh><mesh position={[0, 0.15, 0]} scale={[0.78, 1.4, 0.78]}><sphereGeometry args={[1, 32, 20]} /><meshStandardMaterial color="#222" roughness={0.22} metalness={0.5} /></mesh></group>
}
...
function ModelCanvas({ modelName }: { modelName: string }) {
  const controls = useRef<any>(null)
  const [paused, setPaused] = useState(false)
  const [resetSignal, setResetSignal] = useState(0)
  const resetView = () => { controls.current?.reset(); setResetSignal((value) => value + 1) }
  return <div className={`model-viewer${paused ? ' is-active' : ''}`}>
    <Canvas camera={{ position: [0, 4.6, 0], up: [0, 0, 1], fov: 38 }} dpr={[1, 1.7]} gl={{ antialias: true }}>
      ...
      <OrbitControls ref={controls} target={[0, 0, 0]} enablePan={false} minDistance={1.3} maxDistance={6} autoRotate={false} makeDefault onStart={() => setPaused(true)} onEnd={() => setPaused(false)} />
    </Canvas>
    <div className="viewer-overlay"><span>{paused ? 'Sterowanie aktywne' : 'Przeciągnij, aby obrócić · scroll, aby przybliżyć'}</span><button type="button" onClick={resetView}>Reset widoku</button></div>
  </div>
}

function StaticSkinsScene() {
  return <div className="skins-stage" aria-label="Statyczne modele skinów">
    <Canvas camera={{ position: [0, 6.8, 1.1], up: [0, 0, 1], fov: 32 }} dpr={[1, 1.7]} gl={{ antialias: true }}>
      ...
      <Suspense fallback={null}>
        <group position={[-0.82, 0, 0]} rotation={[0, 0, Math.PI / 2 + Math.PI / 4]} scale={1.05}>
          <DffModel modelName="AzusaGibbous" basePath="/skins" materialColor="#b8b8b2" fallback={<FallbackObject />} />
        </group>
        <group position={[0.82, -0.38, 0]} rotation={[0, 0, Math.PI / 2 + Math.PI / 4 + Math.PI / 12]} scale={1.05}>
          <DffModel modelName="tung" basePath="/skins" materialColor="#b8b8b2" fallback={<FallbackObject />} />
        </group>
...
export default function App() {
  ...
  const chooseModel = (model: (typeof MODELS)[number]) => {
    ...
    setSubject(`Zakup modelu: ${model.label}`)
    setSent(false)
