...
function decodeTxdBitmap(raster: Uint8Array, _rasterFormat: number, _width: number, _height: number) {
  return raster
}
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (txdResponse?.ok) {
          const txdParser = new TxdParser(Buffer.from(await txdResponse.arrayBuffer()))
          const txd = txdParser.parse()
